...
exports.rule = {
  ...
  create: function (context) {
    return {
      "JSXAttribute[name.name='key']": function (pNode) {
...
function checkPropValue(context, node) {
  ...
  if (node.type === 'TemplateLiteral') {
    // key={`foo-${bar}`}
    node.expressions.filter(isGeneratedExpression).forEach(function () {
      context.report({
        messageId: 'noGeneratedKeys',
        node: node,
      });
  ...
  if (node.type === 'BinaryExpression') {
    // key={'foo' + bar}
    var callExpressions = getCallExpressionsFromBinaryExpression(node);
    callExpressions.filter(isGeneratedExpression).forEach(function () {
      context.report({
        messageId: 'noGeneratedKeys',
        node: node,
      });
...
function getCallExpressionsFromBinaryExpression(side) {
  ...
  if (side.type === 'BinaryExpression') {
    // recurse
    var left = getCallExpressionsFromBinaryExpression(side.left);
    var right = getCallExpressionsFromBinaryExpression(side.right);
