    ...
    return {
      CallExpression: function (call) {
        if ((0, regex_1.isStringReplaceCall)(call, services)) {
          var _a = call.arguments,
            pattern = _a[0],
            substr = _a[1];
          var regex = (0, regex_1.getParsedRegex)(pattern, context);
          if (regex !== null) {
            var groups_1 = extractGroups(regex);
            var references = (0, regex_1.extractReferences)(substr);
            var invalidReferences = references.filter(function (ref) {
              return !isReferencingExistingGroup(ref, groups_1);
            });
            if (invalidReferences.length > 0) {
              var groups_2 = ''.concat(invalidReferences.length > 1 ? 's' : '', ': ').concat(
                invalidReferences
                  .map(function (ref) {
                    return ref.raw;
                  })
...
var CapturingGroups = /** @class */ (function () {
  function CapturingGroups() {
    this.names = new Set();
    this.groups = 0;
  }
  CapturingGroups.prototype.add = function (name) {
    if (name !== null) {
      this.names.add(name);
    }
    this.groups++;
  };
  CapturingGroups.prototype.has = function (name) {
    return this.names.has(name);
  };
  CapturingGroups.prototype.count = function () {
    return this.groups;
  };
  return CapturingGroups;
})();
function extractGroups(regex) {
  var groups = new CapturingGroups();
  regexpp.visitRegExpAST(regex, {
    onCapturingGroupEnter: function (group) {
      return groups.add(group.name);
    },
  });
  return groups;
}
function isReferencingExistingGroup(reference, groups) {
  if (!isNaN(Number(reference.value))) {
    var index = Number(reference.value);
    return index >= 1 && index <= groups.count();
  } else {
    var name_1 = reference.value;
    return groups.has(name_1);
  }
}
