    ...
    return {
      ExportDefaultDeclaration: function (node) {
        ...
        if (declaration.type === 'Identifier') {
          var variable = (0, helpers_1.getVariableFromName)(context, declaration.name);
      ...
      'ExportAllDeclaration, ExportNamedDeclaration': function () {
        isOnlyExport = false;
      },
      'Program:exit': function () {
        if (isOnlyExport && nameOfExported) {
          var fileName = path_1.default.parse(context.filename).name;
          if (
            'index' !== fileName &&
...
function sameName(nameOfExported, fileName) {
  var normalizedFileName = fileName.replace(/_/g, '').replace(/-/g, '').replace(/\./g, '');
  var normalizedNameOfExported = nameOfExported.replace(/_/g, '').replace(/-/g, '');
