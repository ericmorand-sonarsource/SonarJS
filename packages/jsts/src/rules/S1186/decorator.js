...
var decorator_1 = require('../S108/decorator');
...
function reportWithQuickFixIfApplicable(context, reportDescriptor) {
  ...
  var functionNode = reportDescriptor.node;
...
function isApplicable(functionNode) {
  ...
  function isMethodDefinition() {
    var methodNode = functionNode.parent;
    return (
  ...
  function isVariableDeclarator() {
    var variableNode = functionNode.parent;
    return (
...
function reportWithQuickFix(context, reportDescriptor, func) {
  var name = reportDescriptor.data.name;
  var openingBrace = context.sourceCode.getFirstToken(func.body);
  var closingBrace = context.sourceCode.getLastToken(func.body);
  (0, decorator_1.suggestEmptyBlockQuickFix)(
    context,
    reportDescriptor,
    name,
    openingBrace,
