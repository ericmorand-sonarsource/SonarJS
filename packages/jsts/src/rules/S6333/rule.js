...
var REST_API_PROPERTIES_POSITION = 2;
var RESOURCE_ADD_RESOURCE_PROPERTIES_POSITION = 1;
var RESOURCE_ADD_METHOD_POSITION = 2;
var REST_API_ROOT_PROPERTY = 'root';
var DEFAULT_METHOD_OPTIONS = 'defaultMethodOptions';
var AUTHORIZATION_TYPE = 'authorizationType';
var NONE_AUTHORIZATION_TYPE = 'aws_cdk_lib.aws_apigateway.AuthorizationType.NONE';
var ADD_METHOD = 'addMethod';
var ADD_RESOURCE = 'addResource';
var GET_RESOURCE = 'getResource';
var PARENT_RESOURCE = 'parentResource';
var messages = {
...
var apigatewayChecker = (0, cdk_1.AwsCdkCheckArguments)(
...
function consumersFactory(ctx) {
  var defaultAuthorizationTypes = new Map();
  var restApiDefaultCollector = defaultsCollector(REST_API_PROPERTIES_POSITION);
  var resourceDefaultCollector = defaultsCollector(RESOURCE_ADD_RESOURCE_PROPERTIES_POSITION);
  return {
    ...
    'aws_cdk_lib.aws_apigateway.RestApi.root': {
      ...
      callExpression: function (expr, _ctx, fqn) {
  ...
  function checkResourceMethod(expr) {
    var properties = (0, result_1.getResultOfExpression)(ctx, expr).getArgument(
      RESOURCE_ADD_METHOD_POSITION,
    );
    var authorizationType = properties.getProperty(AUTHORIZATION_TYPE);
    if (authorizationType.isFound && isSensitiveAuthorizationType(authorizationType.node)) {
      ...
      var defaultAuthorizationType = getDefaultAuthorizationType(expr.callee);
  ...
  function getDefaultAuthorizationType(node) {
    var resource = (0, helpers_1.getUniqueWriteUsageOrNode)(ctx, node);
  ...
  function defaultsCollector(position) {
    return function (expr) {
      var properties = (0, result_1.getResultOfExpression)(ctx, expr).getArgument(position);
      var defaultMethodOptions = properties.getProperty(DEFAULT_METHOD_OPTIONS);
      var authorizationType = defaultMethodOptions.getProperty(AUTHORIZATION_TYPE);
...
function isDefaultFromObject(node) {
  var names = [];
  ...
  return (
    ...
    names.some(function (name) {
      return (0, helpers_1.isMemberWithProperty)(node, name);
    })
  );
}
...
exports.rule = (0, cdk_1.AwsCdkTemplate)(consumersFactory, { meta: { messages: messages } });
