...
exports.rule = {
  ...
  create: function (context) {
    var usedInNew = new Map();
    var usedInCall = new Map();
    var hasIssue = [];
    return {
...
function checkExpression(
  ...
  var variable = getVariable(callExpression, context);
  if (variable && variable.defs.length !== 0) {
    var otherTypeUsage = otherTypeUsageMap.get(variable);
    if (
      (otherTypeUsage === null || otherTypeUsage === void 0 ? void 0 : otherTypeUsage.loc) &&
      !hasIssue.includes(variable)
    ) {
      var message =
        'Correct the use of this function; ' +
        'on line '
          .concat(otherTypeUsage.loc.start.line, ' it was called with')
          .concat(tail, ' "new".');
      context.report({
