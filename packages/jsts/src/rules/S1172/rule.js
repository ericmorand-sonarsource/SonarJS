...
function reportUnusedArgument(node, functionId, context) {
  ...
  if (
    context.getScope().variables.some(function (v) {
      return v.name === 'arguments' && v.identifiers.length === 0 && v.references.length > 0;
    })
  ) {
    return;
  }
  var parametersVariable = context.getDeclaredVariables(node);
  if (functionId) {
    parametersVariable = parametersVariable.filter(function (v) {
      return v.name !== functionId.name;
    });
  }
  for (
    var _i = 0, parametersVariable_1 = parametersVariable;
    _i < parametersVariable_1.length;
    _i++
  ) {
    var param = parametersVariable_1[_i];
    if (
...
function getSuggestions(paramVariable, context) {
  var paramIdentifier = paramVariable.identifiers[0];
  var suggestions = [
    {
      ...
      fix: function (fixer) {
        return fixer.insertTextBefore(paramIdentifier, '_');
      },
    },
  ];
  var func = paramVariable.defs[0].node;
...
function getParameterRemovalSuggestion(func, paramVariable, paramIdentifier, context) {
  return {
    ...
    fix: function (fixer) {
      var paramIndex = func.params.indexOf(paramIdentifier);
      var param = func.params[paramIndex];
      if (func.params.length === 1) {
        var openingParenthesis = context.sourceCode.getTokenBefore(param);
        var closingParenthesis = context.sourceCode.getTokenAfter(param, function (token) {
          return token.value === ')';
        });
        var _a = param.range,
          start = _a[0],
        ...
        var commaAfter = context.sourceCode.getTokenAfter(param, function (token) {
          return token.value === ',';
        });
        var commaBefore = context.sourceCode.getTokenBefore(param, function (token) {
          return token.value === ',';
        });
        var start = commaBefore.range[1];
        var end = param.range[1];
        if (commaAfter) {
        ...
        var start = func.params[paramIndex].range[0];
        var end = func.params[paramIndex + 1].range[0];
        return fixer.removeRange([start, end]);
      }
    },
  };
...
function isParameterProperty(variable) {
  return variable.defs.some(function (def) {
    var _a;
    var parent = def.name.parent;
    return (
      (parent === null || parent === void 0 ? void 0 : parent.type) === 'TSParameterProperty' ||
      ((parent === null || parent === void 0 ? void 0 : parent.type) === 'AssignmentPattern' &&
