  ...
  create: function (context) {
    return {
      'FunctionDeclaration, FunctionExpression, ArrowFunctionExpression': function (node) {
        ...
        for (var _i = 0, _a = functionLike.params; _i < _a.length; _i++) {
          var param = _a[_i];
...
function usesUnionUndefinedOptionalSyntax(node) {
  if (!!node.typeAnnotation && node.typeAnnotation.typeAnnotation.type === 'TSUnionType') {
    var types = node.typeAnnotation.typeAnnotation.types;
    return (
      types.length === 2 &&
      types.some(function (tp) {
        return tp.type === 'TSBooleanKeyword';
      }) &&
      types.some(function (tp) {
        return tp.type === 'TSUndefinedKeyword';
      })
    );
  }
  return false;
