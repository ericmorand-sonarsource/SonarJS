...
(function (Express) {
  ...
  function attemptFindAppInstantiation(varDecl, context) {
    var rhs = varDecl.init;
    if (
      rhs &&
      rhs.type === 'CallExpression' &&
      (0, _1.getFullyQualifiedName)(context, rhs) === EXPRESS
    ) {
      var pattern = varDecl.id;
  ...
  function attemptFindAppInjection(functionDef, context) {
    var app = functionDef.params.find(function (param) {
      return param.type === 'Identifier' && param.name === 'app';
    });
    if (app) {
      var parent_1 = (0, _1.getParent)(context);
      if (
        (parent_1 === null || parent_1 === void 0 ? void 0 : parent_1.type) ===
        'AssignmentExpression'
      ) {
        var left = parent_1.left;
        if (
  ...
  function isUsingMiddleware(context, callExpression, app, middlewareNodePredicate) {
    if ((0, _1.isMethodInvocation)(callExpression, app.name, 'use', 1)) {
      var flattenedArgs = (0, _1.flattenArgs)(context, callExpression.arguments);
  ...
  function isMiddlewareInstance(context, middlewares, n) {
    if (n.type === 'CallExpression') {
      var fqn_1 = (0, _1.getFullyQualifiedName)(context, n);
      return middlewares.some(function (middleware) {
        return middleware === fqn_1;
      });
    }
    return false;
  }
    ...
    return {
      ...
      create: function (context) {
        ...
        function isExposing(middlewareNode) {
          return Boolean(
            sensitiveProperties.push.apply(
              sensitiveProperties,
              findSensitiveProperty(middlewareNode),
            ),
          );
        }
        ...
        return {
          ...
          CallExpression: function (node) {
            if (app) {
              var callExpr = node;
              var isSafe = !isUsingMiddleware(context, callExpr, app, isExposing);
              if (!isSafe) {
                for (
                  var _i = 0, sensitiveProperties_1 = sensitiveProperties;
                  _i < sensitiveProperties_1.length;
                  _i++
                ) {
                  var sensitive = sensitiveProperties_1[_i];
                  context.report({
                    node: callExpr,
          ...
          VariableDeclarator: function (node) {
            if (!app) {
              var varDecl = node;
              var instantiatedApp = attemptFindAppInstantiation(varDecl, context);
              if (instantiatedApp) {
                app = instantiatedApp;
              }
            }
          ...
          ':function': function (node) {
            if (!app) {
              var functionDef = node;
              var injectedApp = attemptFindAppInjection(functionDef, context);
              if (injectedApp) {
                app = injectedApp;
              }
            }
