...
exports.isThisExpression =
  exports.getSimpleRawStringValue =
  exports.isSimpleRawString =
  exports.isStaticTemplateLiteral =
  exports.isObjectDestructuring =
  exports.isIndexNotation =
  exports.isDotNotation =
  exports.isRegexLiteral =
  exports.isNumberLiteral =
  exports.isBooleanLiteral =
  exports.isStringLiteral =
  exports.checkSensitiveCall =
  exports.resolveFunction =
  exports.resolveFromFunctionReference =
  exports.getProperty =
  exports.getPropertyWithValue =
  exports.getObjectExpressionProperty =
  exports.resolveIdentifiers =
  exports.flattenArgs =
  exports.getVariableFromName =
  exports.getVariableFromScope =
  exports.getLhsVariable =
  exports.getValueOfExpression =
  exports.getUniqueWriteUsageOrNode =
  exports.getUniqueWriteReference =
  exports.getUniqueWriteUsage =
  exports.isReferenceTo =
  exports.isElementWrite =
  exports.isUndefined =
  exports.isFalseLiteral =
  exports.isNullLiteral =
  exports.isLiteral =
  exports.isFunctionNode =
  exports.isModuleExports =
  exports.isDefaultSpecifier =
  exports.isNamespaceSpecifier =
  exports.isCallingMethod =
  exports.isMethodCall =
  exports.isFunctionCall =
  exports.isFunctionInvocation =
  exports.isMethodInvocation =
  exports.isRequireModule =
  exports.isArrayExpression =
  exports.isUnaryExpression =
  exports.isBinaryPlus =
  exports.isMemberExpression =
  exports.isMemberWithProperty =
  exports.isIdentifier =
  exports.functionLike =
  exports.FUNCTION_NODES =
    void 0;
exports.isUnresolved = exports.isProperty = void 0;
var _1 = require('.');
exports.FUNCTION_NODES = ['FunctionDeclaration', 'FunctionExpression', 'ArrowFunctionExpression'];
...
function isMemberWithProperty(node) {
  var values = [];
  for (var _i = 1; _i < arguments.length; _i++) {
    values[_i - 1] = arguments[_i];
  }
  return (
    ...
    isIdentifier.apply(void 0, __spreadArray([node.property], values, false))
  );
}
exports.isMemberWithProperty = isMemberWithProperty;
function isMemberExpression(node, objectValue) {
  var propertyValue = [];
  for (var _i = 2; _i < arguments.length; _i++) {
    propertyValue[_i - 2] = arguments[_i];
  }
  if (node.type === 'MemberExpression') {
    var object = node.object,
      property = node.property;
    if (
      isIdentifier(object, objectValue) &&
      isIdentifier.apply(void 0, __spreadArray([property], propertyValue, false))
    ) {
      return true;
    }
  }
...
function isRequireModule(node) {
  var moduleNames = [];
  for (var _i = 1; _i < arguments.length; _i++) {
    moduleNames[_i - 1] = arguments[_i];
  }
...
function isCallingMethod(callExpr, arity) {
  var methodNames = [];
  for (var _i = 2; _i < arguments.length; _i++) {
    methodNames[_i - 2] = arguments[_i];
  }
  return (
...
function isNamespaceSpecifier(importDeclaration, name) {
  return importDeclaration.specifiers.some(function (_a) {
    var type = _a.type,
      local = _a.local;
    return type === 'ImportNamespaceSpecifier' && local.name === name;
  });
}
...
function isDefaultSpecifier(importDeclaration, name) {
  return importDeclaration.specifiers.some(function (_a) {
    var type = _a.type,
      local = _a.local;
    return type === 'ImportDefaultSpecifier' && local.name === name;
  });
}
...
function getUniqueWriteUsage(context, name) {
  var variable = getVariableFromName(context, name);
...
function getUniqueWriteReference(variable) {
  if (variable) {
    var writeReferences = variable.references.filter(function (reference) {
...
function getUniqueWriteUsageOrNode(context, node, recursive) {
  if (recursive === void 0) {
    recursive = false;
  }
  if (node.type === 'Identifier') {
    var usage = getUniqueWriteUsage(context, node.name);
    if (usage) {
...
function getValueOfExpression(context, expr, type, recursive) {
  if (recursive === void 0) {
    recursive = false;
  }
  if (!expr) {
  ...
  if (expr.type === 'Identifier') {
    var usage = getUniqueWriteUsage(context, expr.name);
    if (usage) {
...
function getLhsVariable(context) {
  var parent = context.getAncestors()[context.getAncestors().length - 1];
  var formIdentifier;
...
function getVariableFromScope(scope, name) {
  ...
  while (variable == null && scope != null) {
    variable = scope.variables.find(function (value) {
...
function flattenArgs(context, args) {
  ...
  function recHelper(nodePossiblyIdentifier) {
    var n = getUniqueWriteUsageOrNode(context, nodePossiblyIdentifier);
...
function resolveIdentifiers(node, acceptShorthand) {
  if (acceptShorthand === void 0) {
  ...
  switch (node.type) {
    ...
    case 'ObjectPattern':
      node.properties.forEach(function (prop) {
        return resolveIdentifiersAcc(prop, identifiers, acceptShorthand);
      });
      break;
    case 'ArrayPattern':
      node.elements.forEach(function (elem) {
        return elem && resolveIdentifiersAcc(elem, identifiers, acceptShorthand);
      });
      break;
    case 'Property':
...
function getObjectExpressionProperty(node, propertyKey) {
  if ((node === null || node === void 0 ? void 0 : node.type) === 'ObjectExpression') {
    var properties = node.properties.filter(function (p) {
      return (
        p.type === 'Property' &&
...
function getPropertyWithValue(context, objectExpression, propertyName, propertyValue) {
  var maybeProperty = getObjectExpressionProperty(objectExpression, propertyName);
  if (maybeProperty) {
    var maybePropertyValue = getValueOfExpression(context, maybeProperty.value, 'Literal');
    if (
      (maybePropertyValue === null || maybePropertyValue === void 0
        ? void 0
        : maybePropertyValue.value) === propertyValue
    ) {
...
function getProperty(expr, key, ctx) {
  var unresolvedSpreadElement = false;
  for (var i = expr.properties.length - 1; i >= 0; --i) {
    var property = expr.properties[i];
    if (isProperty(property, key)) {
      return property;
    }
    if (property.type === 'SpreadElement') {
      var props = getValueOfExpression(ctx, property.argument, 'ObjectExpression');
      if (props !== undefined) {
        var prop = getProperty(props, key, ctx);
        if (prop !== null) {
          return prop;
        }
      } else {
...
function resolveFromFunctionReference(context, functionIdentifier) {
  ...
  for (var _i = 0, _a = scopeManager.scopes; _i < _a.length; _i++) {
    var scope = _a[_i];
    var reference = scope.references.find(function (r) {
      return r.identifier === functionIdentifier;
    });
    if (
      (reference === null || reference === void 0 ? void 0 : reference.resolved) &&
...
function checkSensitiveCall(
  ...
  var options = getValueOfExpression(context, sensitiveArgument, 'ObjectExpression');
  if (!options) {
    return;
  }
  var unsafeProperty = getPropertyWithValue(
    context,
    options,
    sensitiveProperty,
    sensitivePropertyValue,
...
function isUnresolved(node, ctx) {
  ...
  if (nodeToCheck.type === 'Identifier') {
    var variable = getVariableFromName(ctx, nodeToCheck.name);
    var writeReferences =
      variable === null || variable === void 0
        ? void 0
        : variable.references.filter(function (reference) {
            return reference.isWrite();
          });
    if (
      !variable ||
      !(writeReferences === null || writeReferences === void 0 ? void 0 : writeReferences.length)
    ) {
      return true;
    }
  }
