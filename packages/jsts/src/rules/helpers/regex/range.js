...
function getRegexpRange(node, regexpNode) {
  ...
  if ((0, __1.isStringLiteral)(node)) {
    ...
    var s = node.raw;
    var tokens = (0, tokenizer_1.tokenizeString)(unquote(s));
  ...
  throw new Error('Expected regexp or string literal, got '.concat(node.type));
}
...
function unquote(s) {
  if (!s.startsWith("'") && !s.startsWith('"')) {
    throw new Error('invalid string to unquote: '.concat(s));
  }
