  ...
  create: function (context) {
    ...
    return {
      ...
      VariableDeclaration: function (node) {
        ...
        var scopeRange = scopeRanges[scopeRanges.length - 1];
        function isOutsideOfScope(reference) {
          var idRange = reference.range;
          return idRange[0] < scopeRange[0] || idRange[1] > scopeRange[1];
        }
        context.getDeclaredVariables(node).forEach(function (variable) {
          var referencesOutside = variable.references
            .map(function (ref) {
          ...
          var definition = variable.defs.find(function (def) {
            return varDeclaration.declarations.includes(def.node);
          });
          if (definition && !reported.includes(definition.name)) {
            context.report({
              node: definition.name,
              message: (0, helpers_1.toEncodedMessage)(
                "Consider moving declaration of '".concat(variable.name, "' ") +
            ...
            variable.defs
              ...
              .forEach(function (defId) {
                return reported.push(defId);
              });
          }
        });
      },
