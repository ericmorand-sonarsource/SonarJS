...
exports.rule = void 0;
var tsTypes = __importStar(require('typescript'));
var helpers_1 = require('../helpers');
var parameters_1 = require('../../linter/parameters');
var message = 'Review this expression to be sure that the concatenation was intended.';
var objectLikeTypes = new Set(['object', 'Object']);
exports.rule = {
  meta: {
    schema: [
      {
  ...
  create: function (context) {
    ...
    function isStringPlusNonString(type1, type2) {
      ...
      var isObjectLike = objectLikeTypes.has(checker.typeToString(type2));
    ...
    function getOperatorLocation(left, right) {
      return context.sourceCode.getTokensBetween(left, right).find(function (token) {
        return token.value === '+' || token.value === '+=';
      }).loc;
    }
    function checkConcatenation(left, right) {
      if (
        (0, helpers_1.isStringLiteral)(left) ||
      ...
      if (
        ...
        context.report({
          message: (0, helpers_1.toEncodedMessage)(
            ...
            [left, right],
            [
              'left operand has type '.concat(checker.typeToString(leftType), '.'),
              'right operand has type '.concat(checker.typeToString(rightType), '.'),
            ],
          ),
