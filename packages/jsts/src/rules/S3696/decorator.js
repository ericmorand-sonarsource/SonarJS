...
function decorate(rule) {
  ...
  return (0, helpers_1.interceptReport)(rule, function (context, reportDescriptor) {
    ...
    if ('node' in reportDescriptor) {
      var thrown_1 = reportDescriptor.node.argument;
      if (isStringLike(thrown_1)) {
        var thrownText_1 = context.sourceCode.getText(thrown_1);
        suggest.push({
          desc: 'Throw an error object',
          fix: function (fixer) {
            return fixer.replaceText(thrown_1, 'new Error('.concat(thrownText_1, ')'));
          },
        });
      }
    }
