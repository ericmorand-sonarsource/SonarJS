...
exports.rule = {
  ...
  create: function (context) {
    ...
    if ((0, helpers_1.isRequiredParserServices)(services)) {
      var returnedExpressions_1 = [];
      return {
        ReturnStatement: function (node) {
          if (returnedExpressions_1.length > 0) {
            returnedExpressions_1[returnedExpressions_1.length - 1].push(node.argument);
          }
        },
        FunctionDeclaration: function () {
          returnedExpressions_1.push([]);
        },
        'FunctionDeclaration:exit': function (node) {
          ...
          if (
            ...
            returnedExpressions_1.length > 0 &&
            allReturnTypesEqual(returnedExpressions_1[returnedExpressions_1.length - 1], services)
          ) {
            context.report({
...
function allReturnTypesEqual(returns, services) {
  var firstReturnType = getTypeFromTreeNode(returns.pop(), services);
  if (!!firstReturnType && !!isPrimitiveType(firstReturnType)) {
    return returns.every(function (nextReturn) {
      var nextReturnType = getTypeFromTreeNode(nextReturn, services);
