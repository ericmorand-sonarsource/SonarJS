...
exports.rule = {
  create: function (context) {
    ...
    function isBuiltInMethod(symbol) {
      var _a;
      var parent = (_a = symbol.valueDeclaration) === null || _a === void 0 ? void 0 : _a.parent;
      if (!parent || parent.kind !== typescript_1.SyntaxKind.InterfaceDeclaration) {
        return false;
      }
      var parentSymbol = tc.getSymbolAtLocation(parent.name);
      if (!parentSymbol) {
        return false;
      }
      var fqn = tc.getFullyQualifiedName(parentSymbol);
    ...
    function declarationMismatch(declaration, callExpression) {
      ...
      for (var i = 0; i < Math.min(parameters.length, callExpression.arguments.length); i++) {
        var parameterType = parameters[i].type;
        if (!parameterType) {
          return null;
        }
        var declaredType = tc.getTypeFromTypeNode(parameterType);
        var actualType = (0, helpers_1.getTypeFromTreeNode)(callExpression.arguments[i], services);
        if (
    ...
    return {
      CallExpression: function (node) {
        ...
        var tsCallExpr = services.esTreeNodeToTSNodeMap.get(callExpression.callee);
        var symbol = tc.getSymbolAtLocation(tsCallExpr);
        if (
          (symbol === null || symbol === void 0 ? void 0 : symbol.declarations) &&
          isBuiltInMethod(symbol)
        ) {
          var mismatch = null;
          ...
          if (mismatch) {
            context.report({
              node: mismatch.node,
              message: "Verify that argument is of correct type: expected '"
                .concat(typeToString(mismatch.declaredType), "' instead of '")
                .concat(typeToString(mismatch.actualType), "'."),
            });
          }
        }
      },
