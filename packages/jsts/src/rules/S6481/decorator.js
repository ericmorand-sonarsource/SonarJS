...
function changeRuleMessagesWith(rule, messageChanger) {
  ...
  if ((_a = rule.meta) === null || _a === void 0 ? void 0 : _a.messages) {
    var messages = rule.meta.messages;
    var newMessages = Object.fromEntries(
      Object.entries(messages).map(function (_a) {
        var key = _a[0],
          value = _a[1];
        return [key, messageChanger(value)];
      }),
    );
...
function lineRemover() {
  var lineRegexp = / \(at line [^)]+\)/g;
  return function (message) {
    return message.replace(lineRegexp, '');
  };
}
