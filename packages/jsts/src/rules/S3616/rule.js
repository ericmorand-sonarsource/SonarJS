...
exports.rule = {
  meta: {
    messages: {
      specifyCase:
        'Explicitly specify {{nesting}} separate cases that fall through; currently this case clause only works for "{{expression}}".',
    },
  },
    ...
    return {
      ...
      'SwitchCase > LogicalExpression': function (node) {
        if (!isSwitchTrue(getEnclosingSwitchStatement(context))) {
          var firstElemAndNesting = getFirstElementAndNestingLevel(node, 0);
