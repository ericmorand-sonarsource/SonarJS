...
exports.rule = (0, regex_1.createRegExpRule)(function (context) {
  return {
    onRegExpLiteralEnter: function (node) {
      node.pattern.alternatives.forEach(function (_a) {
        var elements = _a.elements;
        return checkElements(elements, context);
      });
    },
  };
});
function report(quantifier, context) {
  var ending = quantifier.min === 1 ? '' : 's';
  var message = 'Fix this reluctant quantifier that will only ever match '
    .concat(quantifier.min, ' repetition')
    .concat(ending, '.');
...
function checkElements(elements, context) {
  ...
  var lastButOneElement = elements[elements.length - 2];
