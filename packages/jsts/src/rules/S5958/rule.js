...
exports.rule = {
  create: function (context) {
    var catchWithDone = false;
    function isInsideTest() {
      return context.getAncestors().some(function (n) {
        return n.type === 'CallExpression' && helpers_1.Mocha.isTestConstruct(n);
      });
    }
    return {
      'CatchClause CallExpression[callee.name="done"]': function (_node) {
        catchWithDone = true;
      },
      'CatchClause:exit': function (node) {
        ...
        if (param && param.type === 'Identifier') {
          var exception = (0, helpers_1.getVariableFromIdentifier)(param, context.getScope());
        ...
        if (
          ...
          (callExpr.arguments.length === 0 ||
            (callExpr.arguments.length === 1 &&
              (0, helpers_1.isIdentifier)(callExpr.arguments[0], 'Error')))
        ) {
          context.report({
