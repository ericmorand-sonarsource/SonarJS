...
function checkFunctionLikeDeclaration(functionDeclaration, context) {
  ...
  var returnedExpression = getReturnedExpression(body);
  if (!returnedExpression) {
    return;
  }
  if (isInequalityBinaryExpression(returnedExpression)) {
    var left = returnedExpression.left,
      right = returnedExpression.right;
...
function checkCastedType(node, expression, context) {
  ...
  var castedType = getCastTupleFromMemberExpression(expression);
  if (castedType && castedType[1].type !== 'TSAnyKeyword') {
    var nOfParam = node.params.length;
    if (nOfParam === 1 || (nOfParam === 0 && castedType[0].type === 'ThisExpression')) {
      var castedExpressionText = sourceCode.getText(castedType[0]);
      var castedTypeText = sourceCode.getText(castedType[1]);
      var predicate = ': '.concat(castedExpressionText, ' is ').concat(castedTypeText);
      var suggest = getTypePredicateSuggestion(node, context, predicate);
      context.report({
        messageId: 'useTypePredicate',
        data: {
          castedExpressionText: castedExpressionText,
          castedTypeText: castedTypeText,
        },
...
function getTypePredicateSuggestion(node, context, predicate) {
  ...
  if (node.returnType) {
    fix = function (fixer) {
      return fixer.replaceText(node.returnType, predicate);
    };
  } else {
    var closingParenthesis_1 = context.sourceCode.getTokenBefore(node.body, function (token) {
      return token.value === ')';
    });
    fix = function (fixer) {
      return fixer.insertTextAfter(closingParenthesis_1, predicate);
    };
  }
  suggestions.push({ messageId: 'suggestTypePredicate', fix: fix });
  return suggestions;
}
