...
exports.rule = {
  meta: {
    messages: {
      renameSymbol:
        'Rename this {{symbolType}} "{{symbol}}" to match the regular expression {{format}}.',
    },
  },
  create: function (context) {
    return {
      VariableDeclaration: function (node) {
        return checkVariable(node, context);
      },
      ...
      PropertyDefinition: function (node) {
        return checkProperty(node, context);
      },
      CatchClause: function (node) {
        return checkCatch(node, context);
      },
    };
  },
};
function checkVariable(decl, context) {
  ...
  decl.declarations.forEach(function (declaration) {
    return (0, helpers_1.resolveIdentifiers)(declaration.id).forEach(function (id) {
      return raiseOnInvalidIdentifier(id, 'local variable', context);
    });
  });
}
function checkFunction(func, context) {
  ...
  func.params.forEach(function (param) {
    return (0, helpers_1.resolveIdentifiers)(param).forEach(function (id) {
      return raiseOnInvalidIdentifier(id, 'parameter', context);
    });
  });
}
...
function checkCatch(catchh, context) {
  if (catchh.param) {
    (0, helpers_1.resolveIdentifiers)(catchh.param).forEach(function (id) {
      return raiseOnInvalidIdentifier(id, 'parameter', context);
    });
  }
}
