    ...
    if (node.type === 'CallExpression') {
      ...
      if (callee.type === 'MemberExpression') {
        ...
        if (property.type === 'Identifier' && property.name === 'call') {
          var desc = 'Remove redundant call()';
          if (args.length > 1) {
        ...
        if (property.type === 'Identifier' && property.name === 'apply') {
          var desc = 'Remove redundant apply()',
...
function addFixForCall(suggest, desc, startRange, endRange) {
  if (startRange && endRange) {
    suggest.push({
      desc: desc,
      fix: function (fixer) {
        return fixer.replaceTextRange([startRange[1], endRange[0]], '(');
      },
    });
  }
}
function addFixForCallNoArgs(suggest, desc, startRange, endRange) {
  if (startRange && endRange) {
    suggest.push({
      desc: desc,
      fix: function (fixer) {
        return fixer.replaceTextRange([startRange[1], endRange[1]], '()');
      },
    });
  }
}
function addFixForApply(suggest, desc, argsText, startRange, endRange) {
  if (startRange && endRange) {
    suggest.push({
      desc: desc,
      fix: function (fixer) {
        return fixer.replaceTextRange([startRange[1], endRange[1]], '('.concat(argsText, ')'));
      },
    });
  }
}
