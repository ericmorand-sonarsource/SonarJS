 ...
 */
function initializeLinter(inputRules, environments, globals, linterId) {
  ...
  (0, src_1.debug)(
    'Initializing linter "'.concat(linterId, '" with ').concat(
      inputRules.map(function (rule) {
        return rule.key;
      }),
    ),
  );
  linters[linterId] = new wrapper_1.LinterWrapper({
 ...
 */
function getLinter(linterId) {
  if (linterId === void 0) {
    linterId = 'default';
  }
  if (!linters[linterId]) {
    throw src_1.APIError.linterError(
      'Linter '.concat(linterId, ' does not exist. Did you call /init-linter?'),
    );
  }
