...
exports.PackageJsonsByBaseDir =
  exports.getDependencies =
  exports.getAllPackageJsons =
  exports.getNearestPackageJsons =
  exports.searchPackageJsonFiles =
    void 0;
/*
...
var project_package_json_1 = require('./project-package-json');
var src_1 = require('../../../../shared/src');
var DefinitelyTyped = '@types/';
var PackageJsonsByBaseDir = new project_package_json_1.PackageJsons();
...
function getDependencies(fileName) {
  var dirname = path_1.default.posix.dirname((0, src_1.toUnixPath)(fileName));
  var cached = cache.get(dirname);
  if (cached) {
    return cached;
  }
  var result = new Set();
  cache.set(dirname, result);
  for (var _i = 0, _a = getNearestPackageJsons(fileName); _i < _a.length; _i++) {
    var packageJson = _a[_i];
    dirname = path_1.default.posix.dirname(packageJson.filename);
    var dirCached = dirCache.get(dirname);
    if (dirCached) {
      dirCached.forEach(function (d) {
        return result.add(d);
      });
    } else {
      var dep = getDependenciesFromPackageJson(packageJson.contents);
...
function getDependenciesFromPackageJson(content) {
  ...
  var result = new Set();
  if (content.name) {
    addDependencies(result, ((_a = {}), (_a[content.name] = '*'), _a));
  }
...
function addDependencies(result, dependencies) {
  Object.keys(dependencies).forEach(function (name) {
    return result.add(
      name.startsWith(DefinitelyTyped) ? name.substring(DefinitelyTyped.length) : name,
    );
  });
}
